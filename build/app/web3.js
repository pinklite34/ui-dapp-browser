 ...
 */
export function getWeb3Constructor() {
    var providers = [];
    ...
    evanGlobals.CoreBundle.Web3.providers.WebsocketProvider.prototype.send = function (payload, callback) {
        ...
        if (websocketProvider.connection.readyState !== websocketProvider.connection.OPEN) {
            reconnect(websocketProvider.connection.url, function () {
                _this.send(payload, callback);
            });
            return;
        }
 ...
 */
export function getWeb3Instance(url) {
    try {
        web3 = web3 || new (getWeb3Constructor());
        ...
        if (!websocketProvider || websocketProvider.connection.url !== url) {
            ...
            websocketProvider.on('end', function () { return reconnect(url); });
...
function reconnect(url, callback) {
    if (!reconnecting) {
        ...
        setTimeout(function () {
            ...
            for (var i = 0; i < reconnecting.length; i++) {
                reconnecting[i]();
            }
